...
".$this->get_param('css')) ; 
	...
	*/
	...
	function _modify_content($content, $type, $excerpt) {	
		...
		$postpost = get_post() ; 
		$this->content_for_callback = $postpost->post_content ; // on ne met pas content car il y a beaucoup de lien ajout� par d'autres plugins
		...
		ob_start() ; 
			...
			foreach ($this->table_links as $tl) {
				...
				if (!$tl['anchor']) {
					...
					// Header
					if ($tl['http_code']!=0) {
						if (is_serialized( str_replace("##&#39;##", "'", $tl['header']) )) {
		...
		foreach ($this->table_links as $tl) {
			...
			if (!$tl['anchor']) {
				...
				if ( $result ) {
					...
					// On parcours $tl["occ"] pour ajouter les entr�es qui ne s'y trouvent pas
	...
	public function configuration_page() {
		...
		<div class="plugin-contentSL">		
			...
			ob_start() ; 
				...
				// Tous les resultats qui ont �t� ignor�s
	...
	*/
	function forceAnalysisLinks() {
		...
		if (!empty($at)) {
			...
			// On fait semblant que la page est affich�e pour r�cup�rer les liens des pages
			$postpost = get_post($pid) ; 
	...
	*/
	...
	function checkLinksSynthesis($forced=false, $id_tocheck=-1) {
		...
		foreach ($results as $r) {	
			...
			// Remove the pages that does not exists anymore in occurrence
			$update = "DELETE FROM ".$this->table_name." WHERE ISNULL(occurrence)" ; 
			$wpdb->query($update) ; 
